...
class ants_colony:
    ...
    def __init__(self, nodes, ants_total, distance, iter, pher_dep, pher_adjust, alpha, beta, start=None):
        """
            ...
            @value shortest_dist - the shortest distance in any ant traversal.
            @value shortetst_path - the shortest path corresponding to the shortest_dist
        """
        ...
        """
        self.revised_pher_map = self.init_matrix(len(nodes), 0.0)

        """
            Create a matrix which contains the deposited pher values for the current traversal.
        """
        self.current_pher_map = self.init_matrix(len(nodes), 0.0)
    ...
    def init_ants(self, start):
        ...
        if self.first_time:
            for i in range(self.ants_total):
                ants[i] = self.ant(start, True, list(self.nodes), self.revised_pher_map, self.path_distance, self.alpha, self.beta)
        else:
            i = 0
            for ant in self.ants:
                ants[i] = ant.__init__(start, False, list(self.nodes), self.revised_pher_map, self.path_distance, self.alpha, self.beta)
                i += 1
        return ants
...
if __name__ == "__main__":
    nodes = {0: (0, 3), 1: (4, 8), 2: (10, 5), 3: (12, 10), 4: (9, 13), 5: (13, 8), 6: (7, 12), 7: (13, 11), 8: (10, 11), 9: (11, 8)}
    ...
    colony = ants_colony(nodes, 50, distance, 70, 1000.0, 0.5, 0.5, 1.0)
